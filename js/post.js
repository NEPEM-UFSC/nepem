...
const PostPage = (() => {
  ...
  function parseMarkdown(text) {
    ...
    html = html.replace(/(<li>.*?<\/li>)+/gs, (match) => `<ul class="text-secondary ps-3 my-3" style="list-style-type: disc;">${match}</ul>`);
    ...
    html = paragraphs.map(p => {
      ...
      if (trimmed.startsWith('<ul') || trimmed.startsWith('<div class="table-responsive') || trimmed.startsWith('<h4') || trimmed.startsWith('<h5')) {
        return trimmed;
      }
